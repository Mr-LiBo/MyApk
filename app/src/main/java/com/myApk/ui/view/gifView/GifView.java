...
public class GifView extends View implements GifAction{
	...
	public interface IGifViewListener
	{
		/**
		 * 非主线程，当帧数大于1时有效，首次播放不回调
		 */
		boolean willRunNextLoop();
	}
	...
	private IGifViewListener mListener = null;
	...
	public void setListener(IGifViewListener listener) {
		mListener = listener;
	}
		...
		public void run(){
			...
			while(isRun){
				if(pause == false){
					...
					if (mListener != null && gifDecoder.isLastFrame()
							&& !mListener.willRunNextLoop()){
						isRun = false;
					}
//	    			}else{
//	    				currentImage = gifDecoder.getImage();
