...
/**
 * 抽象的logic建造类<BR>
 * 抽象的logic建造类，用戶緩存logic和集中增加和移除hander<br>
 * 业务层应该使用单例方式继承本类
 *
 * @author 刘鲁宁
    ...
    protected void registerLogic(Class<?> interfaceClass, ILogic logic) {
        if (!mLogicCache.containsKey(interfaceClass.getName())) {
            ...
            if (isInterface(logicClass, interfaceName) && isInterface(logicClass, ILogic.class.getName())) {
                ...
                Logger.d(TAG, "Register logic(" + logicClass.getName() + ") successful.");
            } else {
                Logger.w(TAG, "Register logic(" + logicClass.getName() + ") failed.", new Throwable());
            }
        }
    }
